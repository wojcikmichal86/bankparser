...
def normalize_address(text):
    """Kleinschreibung, Sonderzeichen raus, Mehrfach-Leerzeichen reduzieren"""
    return re.sub(r'\s+', ' ', re.sub(r'[^\w\s]', '', text.lower())).strip()


# --- Adresse matchen ---
def is_address_match(config_inhaber, detected_address):
    """Vergleicht zwei Adressen tolerant: erlaubt Teilstring-Matches"""
...
def generate_booking_text(text_value):
    ...
    # Versuche Name + Ort zu extrahieren
    ...
    # Fallback: nimm die ersten 2–4 Wörter aus dem Text
        ...
        if abs(x0 - target_x) < tolerance and abs(y0 - target_y) < tolerance:
            ...
            for i, line in enumerate(lines):
                if "kontoinhaber" in line.lower():
                    # Alle nachfolgenden Zeilen holen, die kein Stop-Wort enthalten
                    candidate_lines = []
...
# --- Bank- und Adress-Erkennung kombiniert ---
...
def fallback_postfinance_tables(pdf_path):
    ...
    table_columns = [100, 280, 400, 440, 500]  # 6 Spaltenbereiche
        ...
        try:
            ...
            if df_list and df_list[0].shape[0] > 0:
                ...
                if len(df.columns) == 6:
                    ...
                    st.warning(f"⚠️ Unerwartete Spaltenanzahl auf Seite {p}: {len(df.columns)} Spalten")
        except Exception as e:
            st.warning(f"❌ Fehler bei Fallback auf Seite {p}: {e}")
